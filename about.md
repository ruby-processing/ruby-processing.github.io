...
There are two main projects [JRubyArt][jruby_art] and [propane][propane], the most evolved and tested of these is [JRubyArt][jruby_art], which is essentially a version ruby-processing that provides a ruby wrapper around the latest [vanilla processing][processing], and depends on a vanilla processing install (but can be configured to not require a jruby install). [Propane][propane] is a configuration free, standalone version of ruby-processing that depends on an installed jruby (is somewhat more experimental).
